...
import { loginWithSupabase, signInWithOAuth, clearAuthError } from '../../features/auth/authSlice';
...
const LoginModal = ({ onClose, onSwitchToRegister }: LoginModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await dispatch(loginWithSupabase({ email, password }) as any);
    if (!res.error) {
      onClose();
    }
  };
      ...
      >
        ...
        <div className="relative p-6 pb-0">
          ...
          <div className="mb-5">
            ...
            <p className="text-sm text-gray-400">Sign in to your JIGRI account</p>
          </div>
        </div>
